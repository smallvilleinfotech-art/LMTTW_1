  ...
  useEffect(() => {
    async function load() {
      ...
      if (isConfigured && sessionId) {
        try {
          const raw = await fetchLeaderboard(sessionId)
          if (raw && raw.length) {
            data = raw.map(p => ({ ...p, emoji: randomEmoji() }))
          }
        } catch(e) { console.warn('Leaderboard fetch failed:', e) }
      }

      if (!data.length) {
        data = DEMO_LEADERBOARD.map(p => ({ ...p }))
        if (playerScore > 0) {
          data.unshift({
            name:    playerName || 'You 🌟',
            score:   playerScore,
            correct: Math.round(playerScore / 800),
            emoji:   '⭐',
          })
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#46178F] flex items-center justify-center">
        <div className="w-10 h-10 border-[3px] border-white/10 border-t-[#FFA602] rounded-full animate-spin" />
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#46178F] flex flex-col items-center pb-12">
      <div className="w-full max-w-lg px-5 pt-8 animate-fadeUp">
        ...
        <h2 className="font-montserrat font-black text-white text-4xl text-center mb-1">
          🏆 Leaderboard
        </h2>
        <p className="text-white/50 text-sm font-semibold text-center mb-7">
          Top Bible scholars 🕊️
        </p>
        ...
        <div className="flex flex-col gap-2 mb-7">
          {rest.map((p, i) => (
            <div
              key={i}
              className="bg-white/8 border border-white/10 rounded-2xl px-4 py-3
            ...
            >
              ...
              <div className="w-10 h-10 rounded-full bg-[#5B2BA6] border-2 border-white/15
              ...
              <div className="font-montserrat font-black text-[#FFA602] text-xl">
        ...
        <div className="flex flex-col gap-3">
          <KBtn onClick={() => navigate('/host')} variant="yellow">
            🎙️ HOST ANOTHER GAME
          </KBtn>
          <KBtn onClick={() => navigate('/')} variant="ghost">
            🏠 BACK TO HOME
          </KBtn>
        </div>

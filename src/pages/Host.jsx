...
export default function Host() {
  ...
  const currentQidRef = useRef(null)   // quiz id kept in ref for startGame
      ...
      try {
        ...
        rtChanRef.current = supabase
          ...
          .on('postgres_changes', {
            ...
            setPlayers(prev => [...prev, payload.new.name])
          })
          .subscribe()
      } catch(e) {
  ...
  async function endLobby() {
    ...
    if (isConfigured && lobby?.sessionId) {
      await updateSessionStatus(lobby.sessionId, 'finished').catch(() => {})
    }
    setLobby(null)
    setPlayers([])
  }
  ...
  async function startGame() {
    ...
    try {
      if (isConfigured && currentQidRef.current) {
        const qs = await fetchQuestionsForQuiz(currentQidRef.current)
        setQuestions(qs && qs.length ? qs : DEMO_QUESTIONS)
      } else {
      ...
      <div className="min-h-screen bg-[#46178F] flex flex-col items-center pb-10">
        <div className="w-full max-w-xl px-5 pt-8 flex flex-col gap-5 animate-fadeUp text-center">
          ...
          <div>
            ...
            <div className="text-[#FFA602] font-bold text-sm mt-1">{lobby.quizTitle}</div>
          </div>

          {/* PIN */}
          <div className="bg-white rounded-3xl p-7 shadow-2xl">
            <div className="text-xs font-black text-gray-400 tracking-[3px] uppercase mb-2">
              Game PIN — share with players!
            </div>
            <div className="font-montserrat font-black text-[#46178F] text-6xl tracking-[8px] leading-none">
              {lobby.pin}
            </div>
            <div className="text-gray-400 text-xs font-bold mt-3">
              Players enter this PIN on the Home screen
            </div>
          </div>
          ...
          <div className="bg-white/8 border border-white/12 rounded-2xl p-5">
            ...
            <div className="flex flex-wrap gap-2 justify-center min-h-12">
              {players.map((p, i) => (
                <span
                  key={i}
                  className="bg-[#5B2BA6] text-white font-bold text-sm px-4 py-1.5 rounded-full animate-popIn shadow"
                >
                  {p}
                </span>
              ))}
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={endLobby}
              className="flex-1 py-3 bg-red-900/20 border border-red-500/30 rounded-xl
                         text-red-300 font-bold text-sm hover:bg-red-900/35 transition-colors"
            >
              Cancel
            </button>
          </div>
          ...
          <p className="text-white/35 text-xs font-semibold">
            💡 Tip: You can start with 1 player. More can join as long as the game is running.
          </p>
    ...
    <div className="min-h-screen bg-[#46178F] flex flex-col items-center pb-12">
      ...
      <div className="w-full max-w-4xl px-5 pt-8 pb-2 animate-fadeUp">
        ...
        <p className="text-white/50 text-sm font-semibold mt-1">
          Create, launch and manage your Bible quiz games
        </p>
      </div>
      ...
      <div className="w-full max-w-4xl px-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {!loading && filtered.map(q => (
          <QuizCard
            ...
            onEdit={()   => showToast('✏️ Editor coming soon!')}
          />
        ))}
        ...
        {!loading && (
          <button
            ...
            className="border-2 border-dashed border-white/20 rounded-2xl flex flex-col
                       items-center justify-center gap-2 p-10
                       text-white/35 font-montserrat font-black text-base
      ...
      <Modal
        ...
        desc="A 6-digit PIN will be generated. Share it with your players to join."
        confirmLabel="Launch"
...
function QuizCard({ quiz, onLaunch, onDelete, onEdit }) {
  ...
  const questionCount = quiz.questions ?? quiz.questionCount ?? 0
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-md hover:-translate-y-1 hover:shadow-xl transition-all duration-200">
      <div className="h-20 flex items-center justify-center text-5xl relative" style={{ background: bg }}>
        {quiz.emoji}
        <span className="absolute bottom-2 left-2.5 bg-black/40 text-white text-[10px] font-black
                         uppercase tracking-wide px-2 py-0.5 rounded-full">
          {quiz.category}
        </span>
      </div>
      <div className="p-4">
        <h3 className="font-montserrat font-black text-[#1A1A2E] text-base leading-tight mb-1.5">
          {quiz.title}
        </h3>
        <div className="flex gap-4 text-xs font-bold text-gray-400 mb-3">
          <span>❓ {questionCount} questions</span>
          <span>🎮 {quiz.played ?? 0}× played</span>
        </div>
        <div className="flex gap-2">
          <button
            onClick={onLaunch}
            className="flex-1 py-2.5 bg-[#5B2BA6] hover:bg-[#8B60C8] text-white rounded-xl
          ...
          <button
            onClick={onEdit}
            className="flex-1 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-xl
                       text-xs font-black uppercase transition-colors"
          >
            ✏️ Edit
          </button>
          <button
            onClick={onDelete}
            className="flex-1 py-2.5 bg-red-50 hover:bg-red-100 text-red-500 rounded-xl
                       text-xs font-black uppercase transition-colors"
          >
            🗑
          </button>
